...
class Snippet:
    ...
    def __init__(self, lines, provides=None, requires=None, autoscan_requires=True, autoscan_provides=True):
        ...
        stripped_lines = strip_comments_from_lines(self.lines)
...
def scan_use_tags():
    ...
    use_tags_list = sorted(use_tags.keys())

def scan_tags_meta(filename):
    global tags_meta

    with open(filename, 'rb') as f:
        tags_meta = yaml.load(f, Loader=Loader)
...
def get_forced_options(opts):
    ...
    for val in opts.force_options_yaml:
        doc = yaml.load(StringIO(val.decode('utf-8')), Loader=Loader)
        for k in doc.keys():
            if k in use_defs:
                pass  # key is known
            else:
...
def emit_default_from_config_meta(ret, doc, forced_opts, undef_done, active_opts):
    ...
    elif isinstance(defval, numbers.Integral):
        # integer value
        ret.line('#define ' + defname + ' ' + cint_encode(defval))
        active_opts[defname] = True
    elif isinstance(defval, (str, bytes)):
        # verbatim value
