...
def metadata_normalize_shorthand(meta):
    ...
    def decodeStructuredValue(val):
        ...
        if isinstance(val, (numbers.Integral, float, str)):
            return val  # as is
    ...
    for idx, obj in enumerate(meta['objects']):
        for prop in obj['properties']:
            ...
            if not strs_have.get(key):
                ...
                strs_have[key] = True
            if 'value' in prop and isinstance(prop['value'], (str, bytes)):
    ...
    for o in meta['objects']:
        for p in o['properties']:
            ...
            for p2 in targ['properties']:
                ...
                if p2['key'] == 'length' and isinstance(p2['value'], numbers.Integral):
                    lf_len = p2['value']
...
def load_metadata(opts, rom=False, build_info=None, active_opts=None):
    ...
    with open(opts.strings_metadata, 'rb') as f:
        strings_metadata = recursive_strings_to_bytes(yaml.load(f, Loader=Loader))
    with open(opts.objects_metadata, 'rb') as f:
        objects_metadata = recursive_strings_to_bytes(yaml.load(f, Loader=Loader))
    ...
    for fn in opts.builtin_files:
        ...
        with open(fn, 'rb') as f:
            user_meta = recursive_strings_to_bytes(yaml.load(f, Loader=Loader))
...
def gen_ramobj_initdata_for_props(meta, be, bi, string_to_stridx, natfunc_name_to_natidx, objid_to_bidx, double_byte_order):
    ...
    for valspec in values:
        ...
        if val is None:
            ...
            #logger.debug('DOUBLE: %s -> %s' % (struct.unpack("d", val)[0], struct.unpack("d",bytes.fromhex(data))[0]))
...
def main():
    ...
    for s in ram_meta['strings_stridx']:  # XXX: provide all lists?
        ...
        t2 = codecs.encode(unicode_to_bytes(s['str']),'base64').decode('ascii')
